...
from .models import (
    Space, SpaceMembership, FeedPost, Comment, SpaceCategory,
    Question, Answer, KnowledgeHub, KnowledgeHubFeedback, PeerReview,
    Conversation, ConversationParticipant, Message,PeerReviewUpvote, FeedPostLike
)
from accounts.serializers import SimpleUserSerializer # Import the UserSerializer from accounts app
...
class SpaceSerializer(serializers.ModelSerializer):
    ...
    member_count = serializers.SerializerMethodField() # To show how many members a space has
    ...
    class Meta:
        model = Space
        fields = ('id', 'name', 'description', 'admin', 'created_at', 'member_count','members', 'space_profile', 'space_profile_url','category','category_id')
...
class SpaceMembershipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ('user', 'space',) # User and space are set by the view

# --- Feed Post Serializers ---

class FeedPostSerializer(serializers.ModelSerializer):
    author = SimpleUserSerializer(read_only=True) # Nested read-only author details
    space = SpaceSerializer(read_only=True) # Nested read-only space details (if applicable)
    ...
    class Meta:
        model = FeedPost
        fields = ('id', 'author', 'space', 'space_id', 'title', 'content', 'created_at', 'updated_at','like_count','media','share_count','comment_count','is_liked','is_shared')
        read_only_fields = ('author', 'space',) # Author and space are set by the view
...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    space_id = serializers.PrimaryKeyRelatedField(
        queryset=Space.objects.all(), source='space', write_only=True,
        help_text="ID of the space this question belongs to."
    )

    class Meta:
        model = Question
        fields = ('id', 'author', 'space', 'space_id', 'title', 'content', 'is_resolved', 'created_at', 'updated_at','answer_count', 'anticipation_count')
        read_only_fields = ('author', 'space', 'is_resolved',) # Author and space set by view, is_resolved updated via specific action
...
class AnswerSerializer(serializers.ModelSerializer):
    ...
    question_id = serializers.PrimaryKeyRelatedField(
        queryset=Question.objects.all(), source='question', write_only=True, required=False
    )
    ...
    class Meta:
        model = Answer
        fields = (
            'id', 'author', 'question', 'question_id',
            'content', 'is_accepted', 'created_at', 'updated_at', 'upvotes'
        )
...
class KnowledgeHubSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = KnowledgeHub
        fields = ('id', 'teacher', 'space', 'space_id', 'title', 'prompt', 'status', 'created_at', 'updated_at')
        read_only_fields = ('teacher', 'space',) # Teacher and space set by view

    def validate(self, data):
        # Custom validation for private hubs requiring a space
        if data.get('status') == 'private' and not data.get('space'):
            raise serializers.ValidationError("Private knowledge hubs must be associated with a space.")
        return data
...
class KnowledgeHubFeedbackSerializer(serializers.ModelSerializer):
    ...
    hub_id = serializers.PrimaryKeyRelatedField(
        queryset=KnowledgeHub.objects.all(), source='hub', write_only=True,
        help_text="ID of the knowledge hub this feedback belongs to."
    )

    class Meta:
        model = KnowledgeHubFeedback
        fields = ('id', 'student', 'hub', 'hub_id', 'content', 'created_at', 'updated_at')
        read_only_fields = ('student', 'hub',) # Student and hub set by view
...
class PeerReviewSerializer(serializers.ModelSerializer):
    ...
    feedback_to_review_id = serializers.PrimaryKeyRelatedField(
        queryset=KnowledgeHubFeedback.objects.all(), source='feedback_to_review', write_only=True,
        help_text="ID of the knowledge hub feedback being reviewed."
    )

    class Meta:
        model = PeerReview
        fields = ('id', 'reviewer', 'feedback_to_review', 'feedback_to_review_id', 'review_content', 'rating', 'created_at','upvote_count')
        read_only_fields = ('reviewer', 'feedback_to_review',) # Reviewer and feedback set by view
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    conversation = serializers.PrimaryKeyRelatedField(queryset=Conversation.objects.all(), write_only=True) # For creating, client sends conversation ID

    class Meta:
        model = Message
        fields = ('id', 'conversation', 'sender', 'content', 'timestamp')
        read_only_fields = ('sender', 'timestamp',) # Sender and timestamp set by view

class ConversationParticipantSerializer(serializers.ModelSerializer):
    user = SimpleUserSerializer(read_only=True)
    last_read_message = MessageSerializer(read_only=True) # Optional: nested message detail

    class Meta:
        model = ConversationParticipant
        fields = ('id', 'user', 'joined_at', 'last_read_message')
        read_only_fields = ('user', 'last_read_message',)
...
class ConversationSerializer(serializers.ModelSerializer):
    ...
    last_message = serializers.SerializerMethodField() # To show the last message in the conversation

    class Meta:
        model = Conversation
        fields = ('id', 'name', 'is_group_chat', 'created_at', 'updated_at', 'participants', 'participant_ids', 'last_message')
    ...
    def create(self, validated_data):
        ...
        if not is_group_chat and len(set(participant_ids)) > 2:
            raise serializers.ValidationError("Direct messages can only have two participants.")
    ...
    def update(self, instance, validated_data):
        ...
        instance.is_group_chat = validated_data.get('is_group_chat', instance.is_group_chat)
        instance.save()
        return instance
    
class FeedPostLikeSerializer(serializers.ModelSerializer):
    user = SimpleUserSerializer(read_only=True)
    post_id = serializers.PrimaryKeyRelatedField(queryset=FeedPost.objects.all(), source='post', write_only=True)
...
class PeerReviewUpvoteSerializer(serializers.ModelSerializer):
    user = SimpleUserSerializer(read_only=True)
    review_id = serializers.PrimaryKeyRelatedField(queryset=PeerReview.objects.all(), source='review', write_only=True)
