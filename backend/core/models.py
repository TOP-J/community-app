...
class SpaceMembership(models.Model):
    ...
    class Meta:
        unique_together = ('user', 'space') # Ensures a user can't join the same space twice
...
class FeedPost(models.Model):
    ...
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        limit_choices_to={'user_type__in': ['admin', 'teacher']}, # Only admins/teachers can post
...
class Comment(models.Model):
    ...
    upvoters = models.ManyToManyField(CustomUser, related_name='upvoted_comments', blank=True)
...
class Question(models.Model):
    ...
    anticipators = models.ManyToManyField(CustomUser, related_name="anticipated_questions", blank=True)
...
class KnowledgeHubFeedback(models.Model):
    ...
    class Meta:
        ordering = ['created_at']
        unique_together = ('hub', 'student') # A student can only submit one feedback per hub
...
class PeerReview(models.Model):
    ...
    class Meta:
        unique_together = ('reviewer', 'feedback_to_review') # A student can only review a specific feedback once
...
class PeerReviewUpvote(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    review = models.ForeignKey(PeerReview, on_delete=models.CASCADE, related_name='upvotes')
    created_at = models.DateTimeField(auto_now_add=True)
...
class ConversationParticipant(models.Model):
    ...
    class Meta:
        unique_together = ('conversation', 'user') # A user can only be a participant once per conversation
        verbose_name = "Conversation Participant"
        verbose_name_plural = "Conversation Participants"

    def __str__(self):
        return f"{self.user.username} in {self.conversation.name or self.conversation.id}"
