...
from rest_framework.permissions import IsAuthenticated, AllowAny # DRF default permissions
from rest_framework import permissions

from django.db.models import Q,Count # For complex queries
from accounts.models import CustomUser
from .models import (
    Space, SpaceMembership, FeedPost, Comment,
    Question, Answer, KnowledgeHub, KnowledgeHubFeedback, PeerReview,
    Conversation, ConversationParticipant, Message, FeedPostShare
)
from .serializers import (
    SpaceSerializer, SpaceMembershipSerializer, FeedPostSerializer, CommentSerializer,
    QuestionSerializer, AnswerSerializer, KnowledgeHubSerializer, KnowledgeHubFeedbackSerializer,
    PeerReviewSerializer, ConversationSerializer, MessageSerializer, ConversationParticipantSerializer,
    AnswerSerializer, SimpleUserSerializer, 
)
from .permissions import (
    IsAdminUser, IsTeacherOrAdmin, IsStudent, IsOwnerOrAdminOrTeacher,
    IsSpaceMember, IsConversationParticipant
)
...
class SpaceViewSet(viewsets.ModelViewSet):
    ...
    def my_space(self, request):
        ...
        try:
            ...
            return Response({"detail": "User is not a member of any space."}, status=status.HTTP_404_NOT_FOUND)
    ...
    def join(self, request, pk=None):
        ...
        return Response({"detail": f"Successfully joined space: {space.name}"}, status=status.HTTP_200_OK)
    ...
    def leave(self, request, pk=None):
        ...
        try:
            ...
            membership.delete()
            return Response({"detail": f"Successfully left space: {space.name}"}, status=status.HTTP_200_OK)
        except SpaceMembership.DoesNotExist:
            return Response({"detail": "You are not a member of this space."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def top_contributors(self, request, pk=None):
        ...
        serializer = TopContributorSerializer(users, many=True, context={'request': request})
...
class FeedPostViewSet(viewsets.ModelViewSet):
    ...
    def toggle_share(self, request, pk=None):
        ...
        if already_shared:
            return Response({'shared': False})  # sharing again is allowed but not counted
...
def raise_concern(self, request, pk=None):
    ...
    if post.space:
        teachers = CustomUser.objects.filter(
            id__in=post.space.memberships.filter(user__profile__user_type='teacher').values_list('user_id', flat=True)
        )
...
class CommentViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['update', 'partial_update', 'destroy']:
            self.permission_classes = [IsOwnerOrAdminOrTeacher] # Owner can edit/delete, Admin/Teacher can delete
        return super().get_permissions()
    ...
    def toggle_upvote(self, request, pk=None):
        ...
        if user in comment.upvoters.all():
            ...
            return Response({'upvoted': False, 'upvote_count': comment.upvoters.count()})
        else:
...
class QuestionViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['update', 'partial_update', 'destroy']:
            ...
            self.permission_classes = [IsAuthenticated] # Will check object permission in method
        return super().get_permissions()
    ...
    def mark_resolved(self, request, pk=None):
        ...
        if question.author != request.user:
            return Response({"detail": "You do not have permission to mark this question as resolved."},
                            status=status.HTTP_403_FORBIDDEN)
        question.is_resolved = True
        question.save()
        return Response({"detail": "Question marked as resolved."}, status=status.HTTP_200_OK)
    ...
    def toggle_anticipation(self, request, pk=None):
        ...
        return Response({"anticipator_count": question.anticipators.count()}, status=200)
...
class AnswerViewSet(viewsets.ModelViewSet):
    ...
    def mark_accepted(self, request, pk=None):
        ...
        if question.author != request.user:
            return Response({"detail": "You do not have permission to accept an answer for this question."},
                            status=status.HTTP_403_FORBIDDEN)
        ...
        answer.save()
        return Response({"detail": "Answer marked as accepted."}, status=status.HTTP_200_OK)
...
class KnowledgeHubViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['create']:
            ...
            self.permission_classes = [IsOwnerOrAdminOrTeacher] # Teacher (creator) or Admin
        return super().get_permissions()
    ...
    def get_queryset(self):
        ...
        if user.user_type == 'admin':
            ...
            return queryset.filter(
                Q(status='public') |
                (Q(status='private') & Q(space__id=my_space_id))
...
class KnowledgeHubFeedbackViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['create']:
            ...
            self.permission_classes = [IsOwnerOrAdminOrTeacher] # Student (creator), Admin, or Teacher
        return super().get_permissions()
...
class PeerReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['create']:
            ...
            self.permission_classes = [IsOwnerOrAdminOrTeacher] # Reviewer, Admin, or Teacher
        return super().get_permissions()
...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, IsConversationParticipant] # IsConversationParticipant for object-level
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(owner=self.request.user) # Optional: track creator of conversation

    @action(detail=True, methods=['get'], serializer_class=MessageSerializer, permission_classes=[IsAuthenticated, IsConversationParticipant])
    ...
    @action(detail=True, methods=['post'], serializer_class=MessageSerializer, permission_classes=[IsAuthenticated, IsConversationParticipant])
    ...
    def mark_read(self, request, pk=None):
        ...
        try:
            ...
            return Response({"detail": "No messages in conversation to mark as read."}, status=status.HTTP_204_NO_CONTENT)
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, IsOwnerOrAdminOrTeacher] # IsOwnerOrAdminOrTeacher for object-level
...
class FeedPostLikeViewSet(viewsets.ModelViewSet):
    ...
    def toggle(self, request):
        ...
        try:
            ...
            return Response({'detail': 'No FeedPost matches the given query.'}, status=404)
        if post.author == user:
