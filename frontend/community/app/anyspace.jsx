    ...
    try {
      ...
      if (tab === "Questions") {
        const questionsRes = await fetch(
          `https://babaj237.pythonanywhere.com:8000/api/spaces/${spaceId}/questions/`,
          {
        ...
        const postsRes = await fetch(
          `https://babaj237.pythonanywhere.com:8000/api/spaces/${spaceId}/posts/`,
          {
            headers: { Authorization: `Token ${token}` },
          }
        );
    ...
    const fetchData = async () => {
      try {
        ...
        const [spaceRes, contribRes] = await Promise.all([
          fetch(
            `https://babaj237.pythonanywhere.com:8000/api/spaces/${spaceId}/`,
            {
              headers: { Authorization: `Token ${token}` },
            }
          ),
          fetch(
            `https://babaj237.pythonanywhere.com:8000/api/spaces/${spaceId}/top_contributors/`,
            {
              headers: { Authorization: `Token ${token}` },
            }
          ),
