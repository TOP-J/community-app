...
import {Alert, Text, SafeAreaView, Image, StyleSheet, TextInput, View, ActivityIndicator} from "react-native";
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMessage("Login successful", `Welcome, ${username}! Token saved.`, 'success');
        ...
        const errorMessage = errorData.detail || errorData.non_field_errors || "Invalid credentials. Please try again.";
      ...
      if (error instanceof TypeError && error.message.includes('Network request failed')) {
          ...
          try {
              ...
              const errorMessage = errorData.detail || errorData.non_field_errors || "An unexpected error occurred.";
              showMessage("Error", errorMessage, 'error');
              console.error("API Error (JSON parse attempt):", errorData);
          } catch (jsonError) {
              // Fallback if response was not valid JSON
              showMessage("Error", "An unexpected error occurred. Server response was not valid JSON.", 'error');
              console.error("Error parsing JSON response:", jsonError);
              console.error("Raw response text:", response.text ? await response.text() : "No response text");
          }
      } else {
